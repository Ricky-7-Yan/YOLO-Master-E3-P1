...
if not defined E3_P1_RUN_ID (
  for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd-HHmmss-fff"') do set "E3_P1_RUN_ID=repro-strengthened-%%I"
)
echo P1 strengthened benchmark run id: %E3_P1_RUN_ID%
"%E3_PYTHON%" -m e3_p1.strengthened --config "%~dp0configs\p1_strengthened.yaml" --run-id "%E3_P1_RUN_ID%" --no-update-latest
exit /b %ERRORLEVEL%
