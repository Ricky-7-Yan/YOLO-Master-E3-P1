...
if not defined E3_P1_RUN_ID (
  for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd-HHmmss-fff"') do set "E3_P1_RUN_ID=repro-%%I"
)
echo P1 benchmark run id: %E3_P1_RUN_ID%
"%E3_PYTHON%" -m e3_p1 --config "%~dp0configs\p1.yaml" --run-id "%E3_P1_RUN_ID%" --no-update-latest
exit /b %ERRORLEVEL%
